    ...
    for attr in item_struct.attrs.drain(..) {
        if attr.path().is_ident("index") {
            ...
            if !matches!(attr.meta, Meta::Path(_)) {
                return Err(Error::new(attr.span(), "#[track_last_update] takes no arguments"));
            }
...
mod tests {
    ...
    fn track_last_update_ddl_and_insert() {
        ...
        assert!(out.contains("__last_written_ms INTEGER NOT NULL DEFAULT (unixepoch('now') * 1000)"));
        ...
        assert!(!out.contains("SELECT __last_written_ms") && !out.contains("__last_written_ms FROM"));
    }
    ...
    fn track_last_update_compound_pk() {
        ...
        assert!(out.contains("ON CONFLICT(operation_id, position) DO UPDATE SET __last_written_ms"));
    }
